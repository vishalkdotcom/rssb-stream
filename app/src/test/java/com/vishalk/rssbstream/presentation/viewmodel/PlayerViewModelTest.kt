...
class PlayerViewModelTest {
    ...
    private val mockUserPreferencesRepository: UserPreferencesRepository =
            mockk(relaxed = true) // relaxed for flows not directly tested
    ...
    fun setUp() {
        ...
        coEvery { mockUserPreferencesRepository.globalThemePreferenceFlow } returns
                flowOf("Dynamic")
        ...
        coEvery { mockUserPreferencesRepository.albumsSortOptionFlow } returns
                flowOf("AlbumTitleAZ")
        coEvery { mockUserPreferencesRepository.artistsSortOptionFlow } returns
                flowOf("ArtistNameAZ")
        coEvery { mockUserPreferencesRepository.likedSongsSortOptionFlow } returns
                flowOf("LikedSongTitleAZ")

        // Mock repository calls that happen in init
        coEvery { mockMusicRepository.getRecentSearchHistory(any()) } returns emptyList()
        coEvery { mockMusicRepository.getAllUniqueAlbumArtUris() } returns
                emptyList() // For theme preloading
        coEvery { mockMusicRepository.getAudioFiles(any(), any()) } returns
                emptyList() // For initial song load
        coEvery { mockMusicRepository.getAlbums(any(), any()) } returns
                emptyList() // For initial album load
        coEvery { mockMusicRepository.getArtists(any(), any()) } returns
                emptyList() // For initial artist load
    ...
    inner class SearchAndFiltersTests {
        ...
        fun `test_performSearch_callsRepositorySearchAll_andUpdatesState`() = runTest {
            ...
            coEvery { mockMusicRepository.addSearchHistoryItem(query) } just
                    runs // For search history part
            coEvery { mockMusicRepository.getRecentSearchHistory(any()) } returns
                    emptyList() // For search history part
        ...
        fun `test_performSearch_withNonBlankQuery_addsToHistory_andReloadsHistory`() = runTest {
            ...
            coEvery { mockMusicRepository.searchAll(query, any()) } returns
                    emptyList() // Search result itself is not important here
            coEvery { mockMusicRepository.addSearchHistoryItem(query) } just runs
            coEvery { mockMusicRepository.getRecentSearchHistory(any()) } returns
                    listOf(SearchHistoryItem(query = query, timestamp = 0L))
            ...
            coVerify(atLeast = 1) {
                mockMusicRepository.getRecentSearchHistory(any())
            } // Called in init and after adding
            assertEquals(
                    query,
                    playerViewModel.playerUiState.value.searchHistory.firstOrNull()?.query
            )
        }
    ...
    inner class SearchHistoryTests {
        ...
        fun `test_loadSearchHistory_updatesUiState`() = runTest {
            ...
            playerViewModel.playerUiState.test {
                ...
                // For simplicity, we assume this test focuses on a direct call to
                // loadSearchHistory.
                ...
                // we might get intermediate empty lists. Loop until we get the one we expect or
                // fail.
        ...
        fun `test_deleteSearchHistoryItem_callsRepository_andRefreshesHistory`() = runTest {
            ...
            val initialHistory =
                    listOf(
                            SearchHistoryItem(query = queryToDelete, timestamp = 1L),
                            SearchHistoryItem(query = "keep me", timestamp = 2L)
                    )
            ...
            playerViewModel.playerUiState.test {
                ...
                // The number of items to skip might need adjustment based on how many emissions
                // occur
                ...
                coVerify(exactly = 1) {
                    mockMusicRepository.deleteSearchHistoryItemByQuery(queryToDelete)
                }
                coVerify(atLeast = 2) {
                    mockMusicRepository.getRecentSearchHistory(any())
    ...
    inner class GetSongUrisForGenreTests {
        ...
        private val song1 =
                Song(
                        ...
                        artist = "Artist A",
                        genre = "Rock",
                        albumArtUriString = "rock_cover1.png",
                        artistId = dummyArtistId,
                        albumId = dummyAlbumId,
                        contentUriString = dummyContentUri,
        ...
        private val song2 =
                Song(
                        ...
                        artist = "Artist B",
                        genre = "Rock",
                        albumArtUriString = "rock_cover2.png",
                        artistId = dummyArtistId,
                        albumId = dummyAlbumId,
                        contentUriString = dummyContentUri,
                        duration = dummyDuration
        ...
        private val song3 =
                Song(
                        ...
                        artist = "Artist C",
                        genre = "Pop",
                        albumArtUriString = "pop_cover1.png",
                        artistId = dummyArtistId,
                        albumId = dummyAlbumId,
                        contentUriString = dummyContentUri,
                        duration = dummyDuration
        ...
        private val song4 =
                Song(
                        ...
                        artist = "Artist D",
                        genre = "Rock",
                        albumArtUriString = "rock_cover3.png",
                        artistId = dummyArtistId,
                        albumId = dummyAlbumId,
                        contentUriString = dummyContentUri,
                        duration = dummyDuration
        ...
        private val song5 =
                Song(
                        ...
                        artist = "Artist E",
                        genre = "Rock",
                        albumArtUriString = "rock_cover4.png",
                        artistId = dummyArtistId,
                        albumId = dummyAlbumId,
                        contentUriString = dummyContentUri,
                        duration = dummyDuration
        ...
        private val song8 =
                Song(
                        id = "8",
                        title = "Jazz Song With Cover",
                        artist = "Artist H",
                        genre = "Jazz",
                        albumArtUriString = "jazz_cover1.png",
                        artistId = dummyArtistId,
                        albumId = dummyAlbumId,
                        contentUriString = dummyContentUri,
                        duration = dummyDuration
        ...
        private val song9 =
                Song(
                        id = "9",
                        title = "Null Genre Song",
                        artist = "Artist I",
                        genre = null,
                        albumArtUriString = "null_genre_cover.png",
                        artistId = dummyArtistId,
                        albumId = dummyAlbumId,
                        contentUriString = dummyContentUri,
                        duration = dummyDuration
        ...
        private fun setupViewModelWithSongs(songs: List<Song>) {
            coEvery { mockMusicRepository.getAudioFiles(1, any()) } returns
                    songs // Mocks the initial load
            // Re-initialize ViewModel or update state if possible.
            // For simplicity, we rely on the @BeforeEach creating a new ViewModel which will pick
            // this up.
            // If tests need to change songs *after* initial setup, a more complex state
            // manipulation or ViewModel action would be needed.
            // The current PlayerViewModelTest structure re-creates PlayerViewModel for each test in
            // @BeforeEach,
            // so this coEvery in a test-specific setup function called *before* super.setUp() or
            // PlayerViewModel init
            // would be ideal. Or, make PlayerViewModel take songs as a constructor arg for tests.
            // Given the current structure, this coEvery should be placed *before* PlayerViewModel()
            // is called.
            // The @BeforeEach in PlayerViewModelTest calls PlayerViewModel().
            // A workaround is to update the state directly if a method was exposed, or re-mock and
            // re-init.

            // Let's try to re-initialize the viewModel for these tests or ensure this mock is set
            // before PlayerViewModel()
            // For now, we'll assume this coEvery is effective for the PlayerViewModel created in
            // @BeforeEach.
            // This means these tests might need their own @BeforeEach or careful sequencing.
            // The current setup in PlayerViewModelTest has PlayerViewModel initialized with
            // emptyList for songs.
            // We need to update the playerUiState.allSongs.
            // A direct way for testability would be if PlayerViewModel had a method like
            // `setAllSongsForTest(songs: List<Song>)`.
            // Lacking that, we rely on mocking the repository call *before* init.
            // The tests will run within runTest { }, so `playerViewModel` from `setUp` is
            // available.
        ...
        fun `genre with fewer than 3 songs returns all their valid covers`() = runTest {
            ...
            playerViewModel.playerUiState.update {
                it.copy(allSongs = listOf(song1, song3, song2).toImmutableList())
            }
            advanceUntilIdle()
        ...
        fun `genre with songs but no valid URIs returns empty list`() = runTest {
            ...
            val rockUris =
                    playerViewModel.getSongUrisForGenre("Rock") // Only song7 which has empty URI
        ...
        fun `empty allSongs list returns empty list for any genre`() = runTest {
            playerViewModel.playerUiState.update {
                it.copy(allSongs = emptyList().toImmutableList())
            }
            advanceUntilIdle()
...
// e.g., app/src/test/java/com/vishalk/rssbstream/MainCoroutineExtension.kt
...
//
// @ExperimentalCoroutinesApi
// class MainCoroutineExtension(private val testDispatcher: TestDispatcher =
