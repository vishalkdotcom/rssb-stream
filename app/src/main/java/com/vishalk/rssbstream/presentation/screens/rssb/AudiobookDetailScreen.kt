...
fun AudiobookDetailScreen(
    ...
    val audiobook = remember(audiobooks, audiobookId) {
        audiobooks.find { it.id == audiobookId }
    }

    // Fetch chapters if audiobook is found but chapters might not be loaded (though model has them)
    // The ContentViewModel.getAudiobookChapters returns a Flow.
    val chaptersFlow = remember(audiobookId) { contentViewModel.getAudiobookChapters(audiobookId) }
        ...
        CollapsingAudiobookTopBar(
            ...
            onPlayClick = {
                if (chapters.isNotEmpty()) {
                    val allSongs = chapters.map { it.toSong() }
                    playerViewModel.playSongs(allSongs, allSongs.first(), audiobook.title)
                }
            }
        )
    }
...
@Composable
private fun CollapsingAudiobookTopBar(
    audiobook: Audiobook,
    ...
    Box(
        ...
        Box(
            ...
            LargeFloatingActionButton(
                ...
                Icon(Icons.Rounded.PlayArrow, contentDescription = "Play audiobook")
            }
        }
    }
}
